...
const AIStatus = React.forwardRef<HTMLDivElement, AIStatusProps>(
  ({ 
    ...
    const providerPrefix = provider !== 'generic' ? `${provider.charAt(0).toUpperCase() + provider.slice(1)}: ` : ''
    
    return (
      <div
        ref={ref}
        data-slot="ai-status"
        className={cn(aiStatusVariants({ status, size, className }))}
        role="status"
        aria-live="polite"
        {...props}
      >
        {config.showSpinner && (
          <Spinner size="sm" variant={status === 'thinking' ? 'default' : 'secondary'} />
        )}
        {showIcon && IconComponent && !config.showSpinner && (
          <IconComponent className="size-4" />
        )}
        <span>
          {providerPrefix}{displayMessage}
        </span>
      </div>
    )
  },
